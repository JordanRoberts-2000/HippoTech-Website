...
const RisingFallingModel = ({children, active, cancelFunction, fadedBackgroundRef, tailwindStyles}: Props) => {
    return (
        <motion.div ref={fadedBackgroundRef} className={`fixed z-50 w-[100%] h-[100lvh] bg-black/70 left-0 top-0 ${!active && ' pointer-events-none'} opacity-0`}
            ...
            <motion.div variants={varients} animate={active ? 'active' : 'inactive'} 
                    className={'absolute left-[50%] translate-x-[-50%] top-[50%] translate-y-[100vh] ' + tailwindStyles}>
                {children}
            </motion.div>
        </motion.div>
    )
